...
class CoverPreview extends StatefulWidget {
  CoverPreview({required this.user_id, Key? key}) : super(key: key);
  String user_id;
  @override
  _CoverPreviewState createState() => _CoverPreviewState();
}

class _CoverPreviewState extends State<CoverPreview> {
  File? image;
  ...
  Future GetProfileData() async {
    var CoverPhoto = await http.get(Uri.parse(Constants.base_url +
        "get_profile_data.php?user_id=" +
        widget.user_id +
        "&auth_key=" +
        Constants.auth_key));

    var data = json.decode(CoverPhoto.body);

    user_profile = data[0]['user_cover_image'];
    setState(() {});
  }
